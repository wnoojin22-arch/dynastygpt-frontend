"use client";
/**
 * TRADE ADVISOR — beta-gated by NEXT_PUBLIC_CHAT_V2_ENABLED.
 *
 *   flag === "true"  → live chat wired to /chat/v2 SSE pipeline
 *                       (Stages 1+3 → 2 → 4 → 5: regex+Haiku verifier,
 *                        factbook, Sonnet 4.6 streaming, validator).
 *   otherwise        → maintenance placeholder (existing UX preserved).
 */
import { useRef, useEffect } from "react";
import { useTrack } from "@/hooks/useTrack";
import { useChatAdvisorV2 } from "@/hooks/useChatAdvisorV2";
...
function formatAssistantContent(text: string) {
  ...
  const VALUE_KEYWORDS = /\b(value|ranked|score|capital|total|worth|market price|PPG)\b/i;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {lines.map((line, i) => {
        ...
        if (isBullet) {
          return <div key={i} style={{ paddingLeft: 4, lineHeight: 1.6 }}>{displayLine}</div>;
        }
        return <div key={i} style={{ fontWeight: i === 0 ? 600 : 400, lineHeight: 1.6 }}>{displayLine}</div>;
      })}
    </div>
  );
}
...
export default function ChatPanel(props: ChatPanelProps) {
  if (!CHAT_V2_ENABLED) return <MaintenancePanel {...props} />;
  return <LiveChatPanel {...props} />;
}

// ── MAINTENANCE PANEL — flag OFF ──────────────────────────────────────
function MaintenancePanel({ collapsed, onToggle }: ChatPanelProps) {
  if (collapsed) {
    return (
  ...
  return (
    <div style={{
      ...
      borderLeft: `1px solid ${C.gold}30`, backdropFilter: 'blur(20px)', overflow: 'hidden',
    }}>
      <div style={{
        ...
        <button onClick={onToggle} style={{
          background: 'none', border: 'none', color: C.dim, cursor: 'pointer', fontFamily: MONO, fontSize: 15, lineHeight: 1, transition: 'color 0.15s',
        }}
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: C.elevated, border: `1px solid ${C.gold}35`, boxShadow: `0 0 20px ${C.gold}15`,
        }}>
        ...
        <div style={{
          ...
          color: '#000', background: C.gold, padding: '4px 10px', borderRadius: 4,
...
function LiveChatPanel({
  leagueId, owner, ownerId, activeTrade, collapsed, onToggle, injectedMessage,
}: ChatPanelProps) {
  const chat = useChatAdvisorV2({
    leagueId: leagueId || '',
    owner: owner || '',
    ownerId,
    activeTrade: activeTrade ?? null,
  });
  const { messages, input, streaming, messagesEndRef, inputRef, sendMessage, setInput, lastCorrection } = chat;
  ...
  useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [messages, lastCorrection]); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => { if (!collapsed) setTimeout(() => inputRef.current?.focus(), 100); }, [collapsed]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  if (collapsed) {
    return (
      ...
      >
        <div style={{
          width: 32, height: 32, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
          background: `linear-gradient(135deg, ${C.goldDark}, ${C.gold})`, boxShadow: `0 0 16px ${C.gold}30`,
        }}>
          <span style={{ fontFamily: MONO, fontSize: 12, fontWeight: 900, color: '#000' }}>AI</span>
        </div>
        <span style={{
          fontFamily: MONO, fontSize: 10, fontWeight: 800, color: C.gold, letterSpacing: '0.15em', writingMode: 'vertical-rl',
        }}>TRADE ADVISOR</span>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{
            width: 24, height: 24, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
            background: `${C.gold}15`, border: `1px solid ${C.gold}40`, boxShadow: `0 0 16px ${C.gold}20`,
      ...
      <div className="chat-scroll" style={{
        ...
        scrollbarWidth: 'thin', scrollbarColor: `${C.gold}30 transparent`,
      }}>
        <style>{`
        ...
        {messages.length === 0 && !streaming && (
          <div style={{ textAlign: 'center', padding: '20px 8px' }}>
            <div style={{ fontFamily: SERIF, fontSize: 15, color: C.goldBright, marginBottom: 6 }}>Ready to advise</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 4 }}>
            {quickPrompts.map((p, i) => (
              <button key={i} onClick={() => sendMessage(p)} style={{
                ...
                cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s', backdropFilter: 'blur(10px)',
              }}
        ...
        {messages.map((msg, i) => {
          ...
          return (
            <div key={i} style={{
              alignSelf: isUser ? 'flex-end' : 'flex-start', maxWidth: '88%', padding: '9px 12px',
              ...
              <div style={{ fontFamily: SANS, fontSize: 13, color: C.primary, lineHeight: 1.6 }}>
                {msg.content ? (
        ...
        {/* Validator self-correction frame, rendered after the assistant
            reply when severity >= HIGH after freshness floor. */}
        {lastCorrection && lastCorrection.violations.length > 0 && !streaming && (
          <div style={{
            alignSelf: 'flex-start', maxWidth: '92%', padding: '9px 12px',
            borderRadius: '8px',
            background: 'rgba(228,114,114,0.06)',
            border: `1px solid rgba(228,114,114,0.28)`,
            ...
            <div style={{ fontFamily: SANS, fontSize: 12, color: C.primary, lineHeight: 1.55 }}>
              {lastCorrection.text}
            </div>
          </div>
        )}
      ...
      <form onSubmit={handleSubmit} style={{
        ...
        <input
          ...
          style={{
            ...
            background: 'rgba(23,27,40,0.8)', border: `1px solid ${C.border}`,
            color: C.primary, fontFamily: SANS, fontSize: 13, outline: 'none',
