              ...
              <div key={t.trade_id || i}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, fontFamily: SANS, marginBottom: 2 }}>
                    {(() => {
                      const ownerCurrent = isOwnerCurrent(t.owner, (t as any).owner_user_id);
                      const cpCurrent = isOwnerCurrent(t.counter_party, (t as any).counter_party_user_id);
                      return (<>
                        <span
                          onClick={(e) => { e.stopPropagation(); onOwnerClick(t.owner, (t as any).owner_user_id); }}
                          style={{ fontWeight: 700, color: C.primary, cursor: ownerCurrent ? "pointer" : "default", borderBottom: ownerCurrent ? `1px dotted ${C.border}` : "none" }}
                          onMouseEnter={(e) => { if (ownerCurrent) e.currentTarget.style.color = C.gold; }}
                          onMouseLeave={(e) => { e.currentTarget.style.color = C.primary; }}
                        >{t.owner}</span>
                        <span style={{ color: C.dim, fontWeight: 500, margin: "0 5px" }}>↔</span>
                        <span
                          onClick={(e) => { e.stopPropagation(); onOwnerClick(t.counter_party, (t as any).counter_party_user_id); }}
                          style={{ fontWeight: 700, color: C.primary, cursor: cpCurrent ? "pointer" : "default", borderBottom: cpCurrent ? `1px dotted ${C.border}` : "none" }}
                          onMouseEnter={(e) => { if (cpCurrent) e.currentTarget.style.color = C.gold; }}
                          onMouseLeave={(e) => { e.currentTarget.style.color = C.primary; }}
